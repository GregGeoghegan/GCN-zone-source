...
// Copyright (c) 2011-2012 GCN Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #GCoinTEST0\r");
            Send(hSocket, "WHO #GCoinTEST0\r");
        } else {
            ...
            Send(hSocket, strprintf("JOIN #GCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #GCoin%02d\r", channel_number).c_str());
        }
